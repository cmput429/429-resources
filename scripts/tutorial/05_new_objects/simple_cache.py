...
from common import SimpleOpts  # This is just a very thin wrapper around the argparse
                               #  package, so all options that you can apply with argparse
                               #  can apply here to
...
# Now we can add an option for the binary to execute
...
system.l2bus = MyXBar()  # see interconnect.py
...
# NOTE this is where we get to use our option!
system.workload = SEWorkload.init_compatible(options.binary)

process = Process()
process.cmd = [options.binary]  # NOTE and here

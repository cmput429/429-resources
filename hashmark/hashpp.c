...
void lock(hash_tbl * tbl, int index) {
    int success = pthread_mutex_lock(&tbl->local[index]);
    ((success == 0) ? (void) (0) : __assert_fail ("success == 0", "hash.c", 23, __PRETTY_FUNCTION__));
}
void unlock(hash_tbl * tbl, int index) {
    int success = pthread_mutex_unlock(&tbl->local[index]);
    ((success == 0) ? (void) (0) : __assert_fail ("success == 0", "hash.c", 34, __PRETTY_FUNCTION__));
}
...
hash_tbl * ht_create(int size) {
    ...
    table->local = malloc(size*sizeof(pthread_mutex_t));
    for(int i=0;i<size;i++) {
        int success = pthread_mutex_init(&table->local[i], ((void *)0));
        ((success == 0) ? (void) (0) : __assert_fail ("success == 0", "hash.c", 66, __PRETTY_FUNCTION__));
    }
    return table;
}
void ht_free(hash_tbl * tbl) {
    ...
    for(int i=0;i<tbl->size;i++) {
        int success = pthread_mutex_destroy(&tbl->local[i]);
        ((success == 0) ? (void) (0) : __assert_fail ("success == 0", "hash.c", 93, __PRETTY_FUNCTION__));
    }
    free(tbl->local);
    free(tbl);
}

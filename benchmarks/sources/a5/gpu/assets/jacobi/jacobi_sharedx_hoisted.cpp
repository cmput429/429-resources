...
constexpr int      TILE            = 64;   // shared-memory tile for x

// Kernel 2: shared-memory tiling of x (branchless)
template<int TILE_>
__global__ void jacobi_sharedx_hoisted(
    ...
    int n)
{
    ...
    float dot = 0.0f;
    for (int base = 0; base < n; base += TILE_) {
        // cooperative load
        for (int t = threadIdx.x; t < TILE_; t += blockDim.x) {
            int idx = base + t;
            s_x[t] = (idx < n) ? x_old[idx] : 0.0f;
        }
        __syncthreads();

        int hi = (base + TILE_ < n) ? (base + TILE_) : n;
...
int main() {
    ...
    for (unsigned it=0; it<ITERS; ++it) {
        jacobi_sharedx_hoisted<TILE><<<grid, block>>>(A_dev, b_dev, x0_dev, x1_dev, (int)n);
